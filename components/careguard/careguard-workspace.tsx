...
function Metric({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#111923] p-5">
      <p className="text-[11px] font-semibold tracking-[0.16em] text-slate-500">{label}</p>
...
function Panel({ title, kicker, children, className = "" }: { title: string; kicker?: string; children: ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-white/10 bg-[#111923] p-5 ${className}`}>
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-white">{title}</h2>
          {kicker && <span className="mt-1 block text-[10px] font-semibold tracking-[0.16em] text-slate-500">{kicker}</span>}
        </div>
      </div>
      {children}
    </section>
...
function Row({ label, value, tone }: { label: string; value: string; tone?: "good" | "warn" }) {
  return (
    <div className="flex items-center justify-between border-b border-white/5 py-3 text-sm last:border-b-0">
      ...
      <strong className={tone === "good" ? "text-emerald-400" : tone === "warn" ? "text-amber-400" : "text-slate-100"}>{value}</strong>
    </div>
  );
}
...
function HomeMap() {
  return (
    <div className="relative h-[330px] rounded-xl bg-[#0a1119] p-5">
      <div className="grid h-full grid-cols-12 grid-rows-8 gap-3 text-[10px] font-semibold tracking-[0.12em] text-slate-500">
        <div className="col-span-6 row-span-4 rounded-xl border border-emerald-400/30 bg-emerald-400/[0.04] p-3">BEDROOM</div>
        <div className="col-span-5 col-start-8 row-span-4 rounded-xl border border-amber-300/30 bg-amber-300/[0.03] p-3">KITCHEN</div>
        <div className="col-span-6 row-span-4 rounded-xl border border-emerald-400/30 bg-emerald-400/[0.04] p-3">LIVING ROOM</div>
        <div className="col-span-2 col-start-8 row-span-2 rounded-xl border border-red-400/40 bg-red-400/[0.05] p-3">STAIRS</div>
        <div className="col-span-3 col-start-10 row-span-2 rounded-xl border border-amber-300/30 bg-amber-300/[0.03] p-3">BATHROOM</div>
      </div>
      <div className="absolute left-[30%] top-[34%] h-4 w-4 rounded-full bg-cyan-300 shadow-[0_0_25px_rgba(103,232,249,.8)]" />
    </div>
  );
}

export function CareGuardWorkspace() {
  ...
  const [timeline, setTimeline] = useState([
    { time: "14:32", title: "MOVEMENT DETECTED", detail: "Bedroom → Living Room" },
    { time: "14:18", title: "COMMUNICATION REQUEST", detail: "Patient requested water" },
    { time: "13:57", title: "ENVIRONMENTAL SCAN", detail: "No hazards detected" },
  ]);
  ...
  const notifyTelegram = useCallback(async (command: Command, trigger: string) => {
    try {
      await fetch("/api/telegram", {
        ...
        body: JSON.stringify({ command, trigger }),
      });
    } catch (error) {
  ...
  const setCommand = useCallback((command: Command, trigger: string) => {
    ...
    setTimeline((items) => [
      { time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: false }), title: "COMMUNICATION REQUEST", detail: `${command} · ${trigger}` },
      ...items,
    ].slice(0, 6));
  ...
  const handleResult = useCallback((result: { multiFaceLandmarks?: Point[][] }) => {
    ...
    if (calibrationFramesRef.current < 45 && ratio > 0.22) {
      ...
      if (calibrationFramesRef.current === 45) {
        ...
        setGesture(`Calibrating facial baseline… ${Math.round(calibrationFramesRef.current / 45 * 100)}%`);
      }
    }
    ...
    if (ratio < 0.18) {
      ...
      openFramesRef.current = 0;
      if (closeFramesRef.current >= 2) {
        eyesClosedRef.current = true;
        setGesture("Eyes closed");
      }
    } else if (ratio > 0.22) {
  ...
  return (
    <div className="min-h-screen bg-[#080d13] text-white">
      <aside className={`fixed inset-y-0 left-0 z-40 w-[230px] border-r border-white/10 bg-[#0b1118] p-4 transition-transform lg:translate-x-0 ${menu ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center gap-2 border-b border-white/10 px-2 pb-5">
          <div className="grid size-8 place-items-center rounded-lg bg-gradient-to-br from-cyan-300 to-violet-400 text-slate-900"><Sparkles size={15} /></div>
          <div><strong className="block text-sm">CareGuard AI</strong><span className="text-[11px] text-emerald-400">● Systems operational</span></div>
        </div>
      ...
      <div className="lg:pl-[230px]">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-[#080d13]/95 px-4 py-3 backdrop-blur lg:px-6">
          <div className="flex items-center gap-3"><button type="button" className="rounded-lg border border-white/10 p-2 lg:hidden" onClick={() => setMenu((value) => !value)} aria-label="Open navigation"><Menu size={18}/></button><div className="text-sm font-semibold lg:hidden">CareGuard AI</div></div>
          <div className="ml-auto flex items-center gap-5 text-xs text-slate-400"><div><span className="block text-[9px] tracking-widest text-slate-600">CAMERA</span><span>{camera === "live" ? "🟢 Live" : camera === "loading" ? "🟡 Loading" : "🟠 Standby"}</span></div><div className="hidden sm:block"><span className="block text-[9px] tracking-widest text-slate-600">PROCESSING</span><span>Local</span></div><div className="hidden sm:block"><span className="block text-[9px] tracking-widest text-slate-600">CARE ENVIRONMENT</span><span className="text-emerald-400">● Active</span></div><span className="rounded-lg border border-violet-300/20 bg-violet-300/10 px-3 py-2 text-violet-100">◆ Demo mode</span></div>
        </header>

        <main className="mx-auto max-w-[1400px] px-4 py-7 lg:px-7">
          <div className="mb-6 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><h1 className="text-3xl font-semibold tracking-tight lg:text-4xl">{title}</h1><p className="mt-2 text-sm text-slate-500">{subtitle} <span className="text-emerald-400">· AI systems operational</span></p></div><div className="text-right"><strong className="block text-sm">Alex Richardson</strong><span className="text-xs text-slate-500">Severe motor impairment · home monitoring active</span><span className="mt-2 inline-flex rounded-full bg-emerald-400/10 px-3 py-1 text-[10px] font-semibold text-emerald-400">SAFE</span></div></div>

          {tab === "overview" && <Overview timeline={timeline} />}
          {tab === "patient" && <Patient />}
          {tab === "vision" && <Vision video={videoRef} canvas={canvasRef} camera={camera} face={face} gaze={gaze} eye={eyeRatioState} gesture={gesture} lastCommand={lastCommand} blinkCount={blinkCount} start={startCamera} stop={stopCamera} reset={resetVision} test={setCommand} />}
          {tab === "safety" && <Safety emergency={() => setCommand("CALL CAREGIVER", "Emergency protocol")} />}
          {tab === "communication" && <Communication command={(value) => setCommand(value, "Patient interface")} emergency={() => setCommand("CALL CAREGIVER", "Emergency protocol")} />}
        </main>
      </div>
